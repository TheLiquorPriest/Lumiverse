...
Every root WORK round receives a private structured phase-control envelope with the exact current custom phase ID (or `null` when no custom phase is active, including when none are configured or all are skipped), the currently admitted tool names in stable order, and the exact currently materialized open required task IDs. The envelope directs the root to call `complete_turn` by itself after satisfying the current phase exit predicate. Before the final custom phase, success returns `phase_advanced` and continues WORK in the next phase; only completion of the final custom phase—or having no active custom phase after all required completion gates are settled—can finish WORK and begin finalization. Workspace reads accept only `objective`, `constraints`, `tasks`, `records`, `submissions`, `artifacts`, and `summary`.
