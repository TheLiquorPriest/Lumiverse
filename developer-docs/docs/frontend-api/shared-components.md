...
The editor's macro picker lists the host's built-in macro catalog — the built-in subset of what the native Loom editor lists — fetched through Lumiverse's authenticated API (`GET /api/v1/macros?scope=core`). Extension-registered definitions are not listed, including your own: the host records whether a definition is built in, but not which extension owns it, so a public mount is served built-ins only and never receives another extension's macro metadata. (The native editor requests the unscoped catalog, which does include extension macros.) Macros you register still resolve normally during prompt assembly; only the insert menu is scoped. The picker fetches lazily the first time the user opens it, and `refreshMacros()` re-fetches on demand and serializes concurrent refreshes; a failed refresh rejects and leaves the previously loaded list in place. The fetched catalog is validated and independently bounded to 128 categories, 256 macros per category, 32 arguments per macro, 8,192 nodes, 32,768 entries, a nesting depth of 16, 65,536 units per string, and 4 MiB of UTF-8 string data — exceeding a bound rejects the refresh rather than truncating the catalog. A public mount has no contextual preview adapter: it does not resolve macros against a chat, character, persona, or active connection, does not issue preview/generation requests, and does not render the Main-only contextual preview controls.
