  ...
  const favorite = settings.favoriteProfileIds.includes(profile.id)
    return (
      <div key={profile.id} className={clsx(styles.profileRow, active && styles.profileRowActive)}>
        <button type="button" className={styles.profileMain} onClick={() => selectProfile(profile)} disabled={profile.review_required === true} aria-disabled={profile.review_required === true}>
