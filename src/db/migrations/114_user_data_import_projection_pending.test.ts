...
const projectionSql = await Bun.file(join(import.meta.dir, "114_user_data_import_projection_pending.sql")).text();
...
describe("114 user-data projection pending migration", () => {
  ...
  test("is the next migration after the archive import control plane", () => {
    const names = readdirSync(import.meta.dir)
      ...
      .sort();
    expect(names).toContain("113_image_public_provenance.sql");
    expect(names).toContain("114_user_data_import_projection_pending.sql");
    expect(names.indexOf("114_user_data_import_projection_pending.sql"))
      .toBeGreaterThan(names.indexOf("113_image_public_provenance.sql"));
  });
