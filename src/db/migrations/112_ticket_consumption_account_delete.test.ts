...
const baselineSql = await Bun.file(new URL("../baseline.sql", import.meta.url)).text();
const legacySql = await Bun.file(new URL("071_import_consumed_tickets.sql", import.meta.url)).text();
const strictSql = await Bun.file(new URL("109_ticket_consumption_strict.sql", import.meta.url)).text();
const correctionSql = await Bun.file(new URL("112_ticket_consumption_account_delete.sql", import.meta.url)).text();
